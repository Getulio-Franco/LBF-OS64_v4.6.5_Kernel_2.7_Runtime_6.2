...
void Coletar_Slots_Inativos(void) {
    ...
    if (!algum_app_ativo && lblStatus) {
        if (strcmp(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v6.3 [KERNEL 2.7] TRY-EXECP - SYSUTILS]") != 0) {
            strcpy(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v6.3 [KERNEL 2.7] TRY-EXECP - SYSUTILS]");
        }
    }
...
void Setup_Interface_Nativa(void) {
    ...
    lblStatus = gui_create_label((TWinControl*)frmTaskbar, "lblStatus", "LBF-OS RUNTIME v6.3 [KERNEL 2.7] TRY-EXECP - SYSUTILS]");
    if (lblStatus) {
